from tkinter import *
from quiz_brain import QuizBrain

THEME_COLOR: str = "#375362"


class QuizInterface:
    def __init__(self, quiz_brain: QuizBrain) -> None:
        self.quiz: QuizBrain = quiz_brain
        self.window: Tk = Tk()
        self.window.title("Quizz")
        self.window.config(padx=20, pady=20, background=THEME_COLOR)

        self.canvas: Canvas = Canvas(width=300, height=250)
        self.quizz_text: int = self.canvas.create_text(
            150,
            125,
            width=280,
            text="XD",
            fill=THEME_COLOR,
            font=("Arial", 20, "italic"),
        )
        self.canvas.grid(row=1, column=0, columnspan=2)

        true_img: PhotoImage = PhotoImage(file="quizapp/photos/true.png")
        self.true_button: Button = Button(
            image=true_img, highlightthickness=0, command=self.true
        )
        self.true_button.grid(row=2, column=0, pady=50)

        false_img: PhotoImage = PhotoImage(file="quizapp/photos/false.png")
        self.false_button: Button = Button(
            image=false_img, highlightthickness=0, command=self.false
        )
        self.false_button.grid(row=2, column=1)

        self.score_label: Label = Label(text="Score: 0", fg="white", background=THEME_COLOR)
        self.score_label.grid(column=1, row=0)

        self.get_next_question()

        self.window.mainloop()

    def get_next_question(self) -> None:
        self.canvas.config(background="white")
        if self.quiz.still_has_questions():
            self.score_label.config(text=f"Score: {self.quiz.score}")
            q_text: str = self.quiz.next_question()
            self.canvas.itemconfig(self.quizz_text, text=q_text)
        else:
            self.canvas.itemconfig(
                self.quizz_text, text="You've reached the end of the quiz"
            )
            self.true_button.config(state="disabled")
            self.false_button.config(state="disabled")

    def true(self) -> None:
        is_right: bool = self.quiz.check_answer("True")
        self.give_feedback(is_right)

    def false(self) -> None:
        is_right: bool = self.quiz.check_answer("False")
        self.give_feedback(is_right)

    def give_feedback(self, is_right: bool) -> None:
        if is_right:
            self.canvas.config(background="green")
        else:
            self.canvas.config(background="red")
        self.window.after(1000, self.get_next_question)
